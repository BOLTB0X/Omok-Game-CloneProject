...
namespace Client
{
    public partial class MutiplayForm : Form
    {
        ...
        private bool judge() //승리 판정 함수
        {
            for (int i = 0; i < edgeCount - 4; i++) // 가로
                for (int j = 0; j < edgeCount; j++)
                    if (board[i, j] == nowPlayer && board[i + 1, j] ==
                        nowPlayer && board[i + 2, j] == nowPlayer && board[i + 3, j] ==
                        nowPlayer && board[i + 4, j] == nowPlayer)
                        return true;
            for (int i = 0; i < edgeCount - 4; i++) // 세로
                for (int j = 4; j < edgeCount; j++)
                    if (board[i, j] == nowPlayer && board[i, j - 1] ==
                        nowPlayer && board[i, j - 2] == nowPlayer && board[i, j - 3] ==
                        nowPlayer && board[i, j - 4] == nowPlayer)
                        return true;
            for (int i = 0; i < edgeCount - 4; i++) // Y = X 직선
                for (int j = 0; j < edgeCount - 4; j++)
                    if (board[i, j] == nowPlayer && board[i + 1, j + 1] ==
                        nowPlayer && board[i + 2, j + 2] == nowPlayer && board[i + 3, j + 3] ==
                        nowPlayer && board[i + 4, j + 4] == nowPlayer)
                        return true;
        ...
        private void boardPicture_Paint(object sender, PaintEventArgs e)
        {
            ...
            gp.DrawLine(p, rectSize * edgeCount - rectSize / 2, rectSize * edgeCount - rectSize / 2, rectSize * edgeCount - rectSize / 2, rectSize * edgeCount - rectSize / 2); //우측
            p = new Pen(lineColor, 1);

            //대각성 방향으로 이동하면서 십자가 모양의 선 그리기
        ...
        private void mutiplayForm_Load(object sender, EventArgs e)
        {
        ...
        private void enterbutton_Click(object sender, EventArgs e)
        {
            ...
            this.status.Text = "[" + this.roomtextBox.Text + "] 방에 접속하였습니다.";
        }
    }
}
